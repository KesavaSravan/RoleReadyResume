...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const [resumeResponse, scoreResponse] = await Promise.all([
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tailor-resume`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ resumeText, jobDescription }),
        }),
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/score-resume`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ resumeText, jobDescription }),
        })
      ...
      if (err.message.includes('Failed to fetch')) {
        alert('Unable to connect to the server. Please make sure the backend is running on http://localhost:3001');
  ...
  const handleGenerateCoverLetter = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/generate-cover-letter`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ resumeText, jobDescription }),
      });
      ...
      setResult(data.output);
      setScoreData(null); // Cover letters shouldn't show ATS mapping scores
    } catch (err) {
        ...
        <div className="generate-section">
          <div className="generate-container">
            <button
              ...
              disabled={loading || !resumeText || !jobDescription || serverStatus === 'offline'}
